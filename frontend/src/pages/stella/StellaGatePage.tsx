...
type Traffic = { today:{up:number;down:number;total:number}; month:{up:number;down:number;total:number}; total:{up:number;down:number;total:number}; onlineClients:number; estimatedToday?:boolean };
  ...
  return <main className="stella-page">{contextHolder}
    ...
    <section><Typography.Title level={3}>节点控制中心</Typography.Title><Typography.Text type="secondary">导入、重置、切换与流量，所有日常操作都在这里。</Typography.Text>
      <Row gutter={[16,16]} className="stella-grid">
        ...
        <Col xs={24} md={12}><Card title={<><SafetyCertificateOutlined /> 节点重置</>}><Typography.Paragraph type="secondary">节点失效、密钥泄露、端口被封或配置异常时快速恢复。</Typography.Paragraph><Space wrap><Button loading={busy} onClick={() => void run('/panel/api/stella/node/restart')}>轻度：重启服务</Button><Button loading={busy} onClick={() => void run('/panel/api/stella/node/random-port')}>随机更换端口</Button><Button loading={busy} onClick={() => setResetOpen(true)}>普通 / 深度重置</Button></Space></Card></Col>
        ...
        <Col xs={24} md={12}><Card title={<><BarChartOutlined /> 流量统计</>}><Row gutter={8}><Col span={8}><Statistic title={traffic?.estimatedToday ? '今日总流量（实时累计）' : '今日总流量'} value={bytes(traffic?.today.total || 0)} /></Col><Col span={8}><Statistic title="本月总流量" value={bytes(traffic?.month.total || 0)} /></Col><Col span={8}><Statistic title="在线客户端" value={traffic?.onlineClients || 0} /></Col></Row><Divider /><Typography.Text type="secondary">总上传 {bytes(traffic?.total.up || 0)} · 总下载 {bytes(traffic?.total.down || 0)}</Typography.Text></Card></Col>
      </Row>
    </section>
